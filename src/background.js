(function () {
  ...
  var net_url = chrome.extension.getURL("index.html");
  var win_properties = {
    url: net_url,
    type: "popup",
    width: 1200,
  ...
  function beforeRequest(req_details) {
    try {
      console.log(req_details); /** console.trace(); /**/
    } catch (e) {}
    if (post_obj)
      post_obj.postMessage({ type: "beforeRequest", req_details: req_details });
  }
  ...
  function completed(req_details) {
    ...
    if (post_obj)
      post_obj.postMessage({ type: "completed", req_details: req_details });
  }
  ...
  function errorOccurred(req_details) {
    ...
    if (post_obj)
      post_obj.postMessage({ type: "errorOccurred", req_details: req_details });
  }

  function setIcon(active) {
    var icon = active ? "wifi_network_active.png" : "wifi_network.png";
  ...
  chrome.runtime.onConnect.addListener(function (port) {
    try {
      console.log(port); /** console.trace(); /**/
    } catch (e) {}
